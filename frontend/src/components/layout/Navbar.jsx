...
import { Activity, Equal, Pill, ShieldCheck, X } from "lucide-react";
    ...
    <header>
      <nav
        ...
        className={cn(
          ...
          isScrolled ? "pt-2" : "border-b border-emerald-100/70 bg-white/75 pt-0 backdrop-blur-xl"
        )}
      >
        <div
          className={cn(
            ...
            isScrolled &&
              "max-w-5xl rounded-2xl border border-emerald-100 bg-white/80 px-3 shadow-panel backdrop-blur-xl"
          )}
        >
          <div className="relative flex flex-wrap items-center justify-between gap-3 py-3">
            <div className="flex w-full items-center justify-between lg:w-auto">
              ...
              <button
                ...
                className="relative z-20 flex h-10 w-10 cursor-pointer items-center justify-center rounded-xl border border-emerald-100 bg-white text-slate-700 shadow-sm transition hover:bg-emerald-50 lg:hidden"
              >
                <Equal
                  className={cn(
            ...
            <div
              className={cn(
                "hidden w-full flex-wrap items-center justify-end rounded-2xl border border-emerald-100 bg-white/90 p-3 shadow-panel backdrop-blur-2xl lg:m-0 lg:flex lg:w-fit lg:gap-4 lg:border-transparent lg:bg-transparent lg:p-0 lg:shadow-none",
                menuState && "block"
              )}
            >
                  ...
                  {navItems.map((item) => (
                    <li key={item.href}>
                      <a
                        ...
                        className="block text-sm font-medium text-slate-600 transition hover:text-emerald-700"
                      >
                        {item.label}
                      </a>
                    </li>
              ...
              <div className="flex w-full flex-col gap-3 border-t border-slate-100 pt-3 sm:flex-row sm:items-center lg:w-fit lg:border-0 lg:pt-0">
                <div className="hidden items-center gap-2 rounded-full border border-emerald-100 bg-emerald-50 px-3 py-2 text-xs font-medium text-emerald-700 xl:flex">
                  <ShieldCheck className="h-3.5 w-3.5" />
                  Secure health workflow
                </div>
