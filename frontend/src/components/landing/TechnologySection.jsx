      ...
      <div className="section-shell">
        <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {tech.map((item) => (
              ...
              >
                <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-[#0b6b35] shadow-sm transition group-hover:bg-[#0b6b35] group-hover:text-white">
        ...
        <div className="mt-10 flex flex-wrap gap-2">
          {["secure by design", "role-based access", "barangay coordination", "AI assisted"].map((label) => (
            <Badge key={label} variant="outline" className="rounded-full border-[#dbe9d5] bg-[#eef8e9]/70 px-3 py-1 text-[#0b6b35]">
              {label}
            </Badge>
          ))}
        </div>
